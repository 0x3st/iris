...
def get_projection(poly: list, axis:list) -> tuple:
    ...
    Args:
        poly: List of coordinates representing the polygon vertices.
        axis: The axis vector to project onto.
        
    Returns:
        tuple: Minimum and maximum values of the polygon's projection.
    '''
    dots = [ax * vx + ay * vy for vx, vy in poly for ax, ay in [axis]]
    return min(dots), max(dots)
    
def get_perpendicular(v:list) -> tuple:
    '''
    Calculate the normalized perpendicular vector of a given vector.
    
    Args:
        v: The input vector as a tuple (x, y).
...
def get_edges(poly:list) -> list:
    '''
    Get all edge vectors of a polygon.
    
    Args:
        poly: List of coordinates representing the polygon vertices.
        
    Returns:
        list: List of vectors representing the edges of the polygon.
    '''
    edges = []
...
def get_polygon_coordinates(shape: turtle.Turtle) -> list:
    '''
    Get the polygon coordinates of a turtle shape.
    
    Args:
        shape (turtle.Turtle): Turtle object with a polygon shape.
        
    Returns:
        list: List of tuples representing the polygon coordinates.
    '''
...
def box_overlap(poly1:list, poly2:list) -> bool:
	...
	Args:
		poly1 (list[tuple[float, float]]): Coordinates of the first polygon.
		poly2 (list[tuple[float, float]]): Coordinates of the second polygon.
    ...
    for edge in edges1 + edges2:
		...
		# Skip the axis if it is a zero vector
        if axis == (0, 0):
            continue
        
        proj1 = get_projection(poly1, axis)
        proj2 = get_projection(poly2, axis)
